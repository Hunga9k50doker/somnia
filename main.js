  ...
  const { option } = await inquirer.prompt([
    {
      ...
      choices: [
        ...
        { name: "2. Execute Swaps and Send", value: "executeSwaps" },
  ...
  switch (option) {
      ...
      const { faucetChoice } = await inquirer.prompt([
        {
          ...
          choices: [{ name: "1. Official Faucet", value: "officialFaucet" }],
        },
      ]);
    ...
    case "executeSwaps":
      const { swapOption } = await inquirer.prompt([
        {
          ...
          choices: [{ name: "1. SomniaSwap", value: "somniaSwap" }],
        },
      ]);
      if (swapOption === "somniaSwap") {
        const { somniaAction } = await inquirer.prompt([
          {
            ...
            choices: [
              ...
              { name: "2. Claim(Mint) Tokens", value: "claimTokens" },
              { name: "3. Send Tokens", value: "sendTokens" },
            ],
          },
        ]);
        ...
        } else if (somniaAction === "sendTokens") {
          console.log("Launching Send Tokens...".green);
          await runScript("actions/SomniaSwap/send.js");
        }
      }
      await pause();
      break;
      ...
      const { contestChoice } = await inquirer.prompt([
        {
          ...
          choices: [{ name: "1. Participate in Trading Meme Contest", value: "tradingContest" }],
        },
      ]);
